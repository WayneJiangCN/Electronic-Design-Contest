...
u8 EUSCI_A3_TX_BUF[EUSCI_A3_SEND_LEN];   //发送缓冲,最大USART2_MAX_SEND_LEN字节
u8 EUSCI_A3_RX_BUF[EUSCI_A3_REC_LEN];   //接收缓冲,最大EUSCI_A2_REC_LEN个字节.
//接收状态
//bit15，    接收完成标志;1完成
//bit14，    接收到0x0d;1接收到0x0d
//bit13~0，  接收到的有效字节数目
u16 EUSCI_A3_RX_STA=0;       //接收状态标记
...
void EUSCIA3_IRQHandler(void)
{
    ...
    if(status & EUSCI_A_UART_RECEIVE_INTERRUPT_FLAG)
    {
        ...
        if((EUSCI_A3_RX_STA&0x8000)==0)//接收未完成
        {
            if(EUSCI_A3_RX_STA&0x4000) /* 接收到了0xff */
            {
                if(Res!=0x0a)EUSCI_A3_RX_STA=0; /* 接收错误,重新开始 */
                else EUSCI_A3_RX_STA|=0x8000; //接收完成了
            }
            else /* 还没收到0Xff */
            {
                ...
                else
                {
                    ...
                    EUSCI_A3_RX_STA++;
                    if(EUSCI_A3_RX_STA>(EUSCI_A3_REC_LEN-1))EUSCI_A3_RX_STA=0;//接收数据错误,重新开始接收
                }
            }
        }
    }
//    u8 len,i; //接收数据长度
//    //如果接收指令完成
//    if(EUSCI_A3_RX_STA&0x8000)  //判断是否接触屏
//    {
//     len=EUSCI_A3_RX_STA&0x3fff;//得到此次接收到的数据长度
//     for(i=0;i<len;i++)
//    {
//        recevchar(EUSCI_A3_RX_BUF[i]);
//
//    }
//     EUSCI_A3_RX_STA=0;    //清空标志
...
void TJC_Send(char * buf1)
{
  ...
  while (1)
 {
     if (buf1[i]!=0)
    {
     ...
     while((EUSCI_A3->STATW)&0x01);       //等待发送结束
     i++;
    }
    else break; //退出循环
  }
